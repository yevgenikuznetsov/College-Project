...
public class DFSM {
	...
	public String encode() {
		return State.encodeStateSet(new HashSet<State>(states)) + "/" + alphabet.encode() + "/" + transitions.encode()
	 ...
	 * removed.
	 * 
	 * @return DFSM that recognizes the same language as this machine, but has no
	 *         unreachable states.
	 */
	...
	// Traverse the state machine graph in a depth first fashion, fixing the
	// traversal order of the transitions
	// according to the order of the alphabet symbols. Relabel the states with a
	// running integer.
	...
	public boolean compute(String input) {
		...
		while (!input.isEmpty()) {
			...
			state = this.transitions.applyTo(state, s);
		}
